...
/**
 * Port of the GatoClient (PC) GatoAuraX module — the reconstructed Noctyrex
 * KillAura v4.3. Exact settings, order of operations and the 13 rotation
 * modes (engine in GatoAuraXRots.kt).
 *
 * PC reference: Combat/GatoAuraX.cpp + GatoAuraXRots1-4.cpp
 *
 * Relay mapping (documented deviations):
 * - Attack: the PC uses GameMode::attack natively; mobile sends the
 *   ITEM_USE_ON_ENTITY transaction (actionType 1 = attack) — same packet the
 *   engine emits. NoSwing suppresses the swing arm packet.
 * - Rain detection: LevelEventPacket START/STOP RAINING instead of engine
 *   weather; water detection: AUTO_JUMPING_IN_WATER input flag instead of the
 *   5x... block scan; the sky scan for riptide has no voxel equivalent.
 * - Switch "Full" changes the local slot in memory on PC; mobile cannot move
 *   the real client's hotbar, so Full/Silent both report the trident to the
 *   server via MobEquipmentPacket (Silent restores the reported slot after).
 * - Rotations spoof the server-bound PlayerAuthInputPacket (and MovePlayer
 *   for client-auth servers); the local camera cannot be moved from the relay.
 */
class GatoAuraXModule : Module("GatoAuraX", ModuleCategory.Combat) {
    ...
    // --- Settings: same names, order, defaults and ranges as the PC ctor ---
    ...
    // last touch input (mobile equivalent of RotIsKeyDown)
    private var lastInput: Set<PlayerAuthInputData> = emptySet()

    // weather (mobile equivalent of the engine rain state)
    ...
    private fun updateTargets(localPlayer: com.gato.client.game.entity.LocalPlayer) {
        targets.clear()
        for (entity in session.level.entityMap.values) {
            ...
            if (!isPlayer && !(attackMobs && isMob)) continue
    ...
    private fun attackTarget(target: Entity) {
        ...
        repeat(multiplier) {
            ...
            if (noSwing) {
                // attack transaction without the swing arm packet
    ...
    override fun beforePacketBound(interceptablePacket: InterceptablePacket) {
        ...
        when (packet) {
            ...
            is PlayerAuthInputPacket -> {
                ...
                // spoof con la rotacion fresca de este tick (mismo orden que PC:
                // onNormalTick calcula, el paquete saliente la lleva)
                spoofRotation(packet)
            }

            is MovePlayerPacket -> {
                // client-auth servers: spoof own movement rotation (serverbound)
    ...
    private fun spoofRotation(packet: PlayerAuthInputPacket) {
        ...
        val m = rotationMode
        if (m < 1 || m == 3) return // None y Strafe no spoofean
    ...
    private fun tick(packet: PlayerAuthInputPacket) {
        ...
        // --- tridente: lluvia y/o agua (sustitutos de relay, ver class comment) ---
        ...
        if (tridentSlot == -1 && switchMode > 0) tridentSlot = findTridentSlot() // fallback del original
        ...
        // --- trident switch (Full/Silent = MobEquipment; Silent restaura) ---
        ...
        // aim vertical: offset para targets aereos
        ...
        // --- dispatcher de modos (orden exacto del binario) ---
        val m = rotationMode
        if (m != 3 && m > 0 && m < 13) {
            rotOut[0] = fpitch // pre-store
            rotOut[1] = fyaw
            headYawOut = fyaw
        }
        ...
        when (m) {
            0 -> {} // None
            3 -> { // Strafe (inline en el original): sway temporal
                val t = System.nanoTime() / 1e9f * 0.1f
                rotOut[0] = fpitch + sin(t) * 10f
                rotOut[1] = fyaw + cos(t) * 10f
            }
            else -> {
        ...
        if (didSwitch && switchMode == 2 /* Silent */) {
            // restaura el slot reportado al servidor
            session.serverBound(MobEquipmentPacket().apply {
                runtimeEntityId = localPlayer.runtimeEntityId
    ...
    private fun sin(f: Float): Float = kotlin.math.sin(f)
    private fun cos(f: Float): Float = kotlin.math.cos(f)
}
